...
namespace UWB_Texturing
{
    public static class CustomMesh
    {
        ...
        public static void SaveMesh(Mesh[] meshes)
        {
            if (meshes != null && meshes.Length > 0)
            {
                ...
                for (int i = 0; i < meshes.Length; i++)
                {
                    ...
                    sb.Append(Separator);
                    sb.Append(MeshID + CrossPlatformNames.Mesh.CompileMeshName(i) + '\n');
        ...
        {
            if (!Directory.Exists(CrossPlatformNames.Mesh.AbsoluteAssetFolder))
            {
                Directory.CreateDirectory(CrossPlatformNames.Mesh.AbsoluteAssetFolder);
            }
            ...
            { 
                ...
                if (MeshRead)
                {
                    ...
                    if (saveAsAsset)
                    {
                        AssetDatabase.CreateAsset(m, CrossPlatformNames.Mesh.CompileAssetPath(m.name));
            ...
            mesh.name = CrossPlatformNames.RoomObject.GameObjectName;
        ...
        {
            int numMeshes = 0;
            string[] files = Directory.GetFiles(CrossPlatformNames.Mesh.AbsoluteAssetFolder);
