...
//        }
        ...
        {
            if (!Directory.Exists(CrossPlatformNames.Material.AbsoluteAssetFolder))
            {
                Directory.CreateDirectory(CrossPlatformNames.Material.AbsoluteAssetFolder);
            }
...
#if UNITY_EDITOR
                AssetDatabase.CreateAsset(roomMat, CrossPlatformNames.Material.CompileAssetPath(roomMat.name));
                AssetDatabase.SaveAssets();
#endif
            }
        ...
        {
            Material mat = new Material(Shader.Find(CrossPlatformNames.Shader.ShaderName_Full));
            mat.name = CrossPlatformNames.Material.CompileAssetName(roomChildIndex);
        ...
        public static Material GetRoomMaterial(int index)
        {
            ...
            string assetName = CrossPlatformNames.Material.CompileAssetName(index);

            Material roomMat = Resources.Load(CrossPlatformNames.Material.GetResourcesLoadPath(assetName)) as Material;
        ...
        public static List<string> GetTextureFiles()
        {
            ...
            foreach (string filepath in Directory.GetFiles(CrossPlatformNames.AssetBundle.InputFolder))
            {
                if (Path.GetExtension(filepath).Equals(CrossPlatformNames.Images.Extension))
                {
        ...
        {
            int numMaterials = 0;
            string[] files = Directory.GetFiles(CrossPlatformNames.Material.AbsoluteAssetFolder);
